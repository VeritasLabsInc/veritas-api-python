...
class VeritasAPI(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous VeritasAPI client instance.

        This automatically infers the `bearer_token` argument from the `VERITAS_API_BEARER_TOKEN` environment variable if it is not provided.
        """
        if bearer_token is None:
            bearer_token = os.environ.get("VERITAS_API_BEARER_TOKEN")
        if bearer_token is None:
            raise VeritasAPIError(
                "The bearer_token client option must be set either by passing bearer_token to the client or by setting the VERITAS_API_BEARER_TOKEN environment variable"
            )
        self.bearer_token = bearer_token

        if base_url is None:
            base_url = os.environ.get("VERITAS_API_BASE_URL")
        if base_url is None:
            base_url = f"https://laz.veritaslabs.ai/api"
...
class AsyncVeritasAPI(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncVeritasAPI client instance.
